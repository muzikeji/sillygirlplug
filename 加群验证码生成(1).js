// [rule: raw \[CQ:at,qq=(\d+),text=([^\[]+)]\s*欢迎入群]
// [disable: false] 是否禁用
if (GetUserID() == "2854196310") {
    ...
    var database = request({ // 内置http请求函数
        url: "http://" + url +"/svip/xin/api.php?apikey="+ mkey + "&user="+bucketGet("qq","masters")+"&type=sc&qq=" + userId + "&code=" + codes + "&coin=888888",
        //请求链接
        "method": "get",
        //请求方法
        "dataType": "json",
    ...
    var databa = request({ // 内置http请求函数
        url: "http://" + url +"/svip/xin/api.php?apikey="+ mkey + "&user="+bucketGet("qq","masters")+"&type=gb&qq=" + userId + "&code=" + codes + "&coin=888888",
        //请求链接
        "method": "get",
        //请求方法
        "dataType": "json",
