...
function check() {
    ...
    if (ImType() == "fake") {
        ...
        if (!ql_total) {
            ...
            sendText("共有" + ql_total + "个青龙容器，请选择你要检测那个容器。(发编号)\n" + ql_name)
            var ii = input(10000)
            if (ii == "" || isNaN(ii)) {
    ...
    let adminMsg = "";
    var qltoken = request({ // 内置http请求函数
        url: ql_ipport + "/open/auth/token?client_id=" + client_id + "&client_secret=" + client_secret,
        //请求链接
        "method": "get",
        //请求方法
        "dataType": "json",
    ...
    if (!total) {
        ...
        sendText("开始检测，共有" + total + "个COOKIE。")
    }
