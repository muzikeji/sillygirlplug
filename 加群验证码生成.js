// [rule: raw \[CQ:at,qq=(\d+),text=([^\[]+)]\s*欢迎入群([\s\S]*)]
//[priority: 520]优先级
// [disable: false] 是否禁用
//[imType:qq] 白名单
function main() {
if (GetUserID() == "2854196310") {
    ...
    var database = request({ // 内置http请求函数
        url: "http://" + url +"/svip/" + mkey + ".php?type=sc&qq=" + userId + "&code=" + codes + "&tg=888888",
        //请求链接
        "method": "get",
        //请求方法
        "dataType": "json",
    ...
    var databa = request({ // 内置http请求函数
        url: "http://" + url +"/svip/" + mkey + ".php?type=gb&qq=" + userId + "&code=" + codes + "&tg=888888",
        //请求链接
        "method": "get",
        //请求方法
        "dataType": "json",
...
var img = "https://api.muvip.cn/api/qqda/?name=" + userName + "&topic1=" + a + "&topic2=" + b + "&times=90"
    sendImage(img)
    sleep(60000)
    var database = request({ // 内置http请求函数
...
var img = "https://api.muvip.cn/api/qqda/?name=" + userName + "&topic1=" + a + "&topic2=" + b + "&times=30"
    sendText("[CQ:at,qq=" + userId + ",text=@" + userName + "] \n题目见下图：\n" + image(img))
        ...
        if (zcode == "1" || zcode == "0") { //已验证
            ...
            if (zcode == "1" || zcode == "0") { //未注册
                ...
                var database = request({ // 内置http请求函数
                    url: "http://" + url +"/svip/" + mkey + ".php?type=kill&qq=" + userId,
                    //请求链接={
                    "method": "get",
                    //请求方法
                    "dataType": "json",
